...
> https://www.gerritcodereview.com/
	...
	# 准备工作目录和身份认证文件
	mkdir /mnt/gerrit
	chown apache:apache /mnt/gerrit
	chmod 777 /mnt/gerrit
	...
	java -jar gerrit-2.13.5.war init --batch -d /mnt/gerrit/playground

	# 修改 /etc/httpd/conf.d/gerrit.conf 文件：
```
Listen 82
<VirtualHost 1.2.3.4:82>
	...
	ProxyPass /playground/ http://localhost:8082/
</VirtualHost>
```

	# 修改 /mnt/gerrit/playground/etc/gerrit.config 文件
```
[gerrit]
	basePath = git
	serverId = bc41c985-ca83-4c35-ad74-7064b3f26193
	canonicalWebUrl = http://1.2.3.4:82/playground/
[database]
	type = h2
	database = /mnt/gerrit/playground/db/ReviewDB
[auth]
	type = HTTP
[receive]
...
```
	...
	# 重启 Gerrit 和 Apache
	/mnt/gerrit/playground/bin/gerrit.sh restart
	/etc/rc.d/init.d/httpd restart

	# 访问 Gerrit 网页
	http://1.2.3.4:82/playground/
