...
> https://www.openssl.org/source/
	...
	cd openssl-1.1.0
	./config --prefix=/mnt/bin/openssl-1.1.0 -fPIC no-shared
	make
	make install
...
> http://dev.mysql.com/downloads/mysql/
	...
	tar -xvzf mysql-5.5.48.tar.gz
	cd mysql-5.5.48
	mkdir bld
	cd bld
	cmake .. -DCMAKE_INSTALL_PREFIX=/mnt/bin/mysql-5.5.48 -DMYSQL_DATADIR=/mnt/data/test
	make
	make install
...
#### mysql -- 用于本机运行 mysql server
	...
	# 创建数据库目录
	cd /mnt/bin/mysql-5.5.48
	scripts/mysql_install_db --user=mysql --datadir=/mnt/data/test

	# 生成配置文件
	cp /mnt/bin/mysql-5.5.48/support-files/my-default.cnf /mnt/sites/config/my.cnf
	...
	# 生成启动脚本
	cp /mnt/bin/mysql-5.5.48/support-files/mysql.server /mnt/sites/config/mysql.server
	...
	# 设置数据库中 mysql 用户的密码：
	/mnt/bin/mysql-5.5.48/bin/mysql --user=root --password= --socket=/tmp/mysql-test.sock -e "GRANT ALL PRIVILEGES ON *.* TO 'root'@'localhost' IDENTIFIED BY '***' WITH GRANT OPTION;"
	/mnt/bin/mysql-5.5.48/bin/mysql --user=root --password=*** --socket=/tmp/mysql-test.sock -e "GRANT ALL PRIVILEGES ON *.* TO 'root'@'%' IDENTIFIED BY '***' WITH GRANT OPTION;"
	/mnt/bin/mysql-5.5.48/bin/mysql --user=root --password=*** --socket=/tmp/mysql-test.sock -e "GRANT ALL PRIVILEGES ON *.* TO 'root'@'127.0.0.1' IDENTIFIED BY '***' WITH GRANT OPTION;"
	/mnt/bin/mysql-5.5.48/bin/mysql --user=root --password=*** --socket=/tmp/mysql-test.sock -e "GRANT ALL PRIVILEGES ON *.* TO 'root'@'::1' IDENTIFIED BY '***' WITH GRANT OPTION;"
...
> https://secure.php.net/downloads.php
	...
	# 下载 PHP 源代码
	cd /mnt/src
	wget http://mirrors.sohu.com/php/php-5.6.5.tar.gz
	...
	touch ac*
	./buildconf --force
	./configure --prefix=/mnt/bin/php-5.6.5 --with-fpm-user=daemon --with-fpm-group=deploy --with-config-file-path=/mnt/bin/php-5.6.5 --with-pdo-mysql=/mnt/bin/mysql-5.5.48 --with-mysql=/mnt/bin/mysql-5.5.48 --with-openssl=/mnt/src/openssl-1.1.0 --enable-soap --with-zlib --with-curl --with-gd --with-jpeg-dir=/usr/lib --with-freetype-dir=/usr/lib --enable-shmop --enable-sockets --enable-mbstring --enable-zip --with-pic --enable-fpm --enable-xcache --with-ssh2 --enable-redis --with-gnupg
	make clean
	make
	make install
	ln -s /mnt/bin/php-5.6.5/bin/php /usr/bin/php

	# 获取 cacert.pem 文件
	wget http://curl.haxx.se/ca/cacert.pem -O /mnt/sites/config/cacert.pem

	# 生成配置文件
	cp /mnt/src/php-5.6.5/php.ini-production /mnt/sites/config/php.ini
	...
	# 修改 /mnt/sites/config/php.ini 文件，关注以下内容：
		...
		expose_php = Off
		short_open_tag = On
		extension_dir = "/mnt/bin/php-5.6.5/ext/"
		xcache.size = 128M
	...
	# 创建 php-fpm 配置文件
	cp /mnt/bin/php-5.6.5/etc/php-fpm.conf.default /mnt/sites/config/php-fpm.conf
...
> http://tengine.taobao.org/download_cn.html
	...
	cd tengine-2.1.2
	./configure --prefix=/mnt/bin/tengine-2.1.2 --conf-path=/mnt/sites/config/tengine/nginx.conf --user=daemon --group=deploy --with-pcre=/mnt/src/pcre-8.36/ --with-openssl=/mnt/src/openssl-1.1.0 --with-http_spdy_module --with-http_concat_module --with-http_lua_module --with-luajit-inc=/mnt/bin/luajit-2.0.4/include/luajit-2.0 --with-luajit-lib=/mnt/bin/luajit-2.0.4/lib
	make
	make install
